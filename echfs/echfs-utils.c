...
static int master_boot_record_partition = 0;
...
int main(int argc, char **argv)
{
    ...
    while((option = getopt(argc, argv, "vmp:")) != -1)
    {
        switch(option)
        {
            case 'v':
            ...
            case 'p':
            {
                master_boot_record_partition = atoi(optarg);
                break;
            }

            default:
            {
                fprintf(stderr, "Usage: %s <options> [image] <action> <arguments...>\n", argv[0]);
            }
        }
    }
    ...
    // if using master boot record
    if(master_boot_record)
    {
        struct master_boot_record_partition master_boot_record_data = master_boot_record_get_partition(image, master_boot_record_partition);
        partition_offset = master_boot_record_data.first_sector * 512;
        image_size = master_boot_record_data.sector_count * 512;
    }
    else
    {
        // set us at the first partition
        partition_offset = 0;
